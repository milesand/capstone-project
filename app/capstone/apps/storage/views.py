...
def perm_check_dir_with_teams(user, directory):
    ...
    user_teams = user.teamList.all().union(user.leader.all()).distict().only("pk")
...
class DirectoryView(APIView):
    ...
    def delete(self, request, pk):
        try:
            ...
            if not perm_check_entry_with_teams(request.user, user):
                raise Directory.DoesNotExist
        except Directory.DoesNotExist:
            return Response(
                {"message": "Directory not found"},
...
class FileDownloadAPI(APIView):
    ...
    def post(self, request):
        ...
        if len(request.data) == 1: # 파일 1개
            try:
                file_record = get_object_or_404(File, pk=request.data['file1'])

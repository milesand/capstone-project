...
오늘 작업한 내용은 아래와 같습니다.
    ...
    1. 공유 폴더 기능 추가. 사용방법은 다음과 같습니다.
     
        1. 가지고 있는 폴더에 오른쪽 클릭 -> 공유 설정 -> 공유 설정할 팀 1개 이상 선택 후 결정 클릭, 여기서 현재 공유설정 중인 폴더 해제도 가능함.
        2. 왼쪽의 공유파일 탭을 누르면 팀 선택 리스트가 나오고, 여기서 팀을 선택하면 해당 팀이 공유중인 폴더 목록이 나온다.
        3. 폴더를 더블클릭하면 공유폴더에 접근 가능하며, 서브폴더 생성, 업로드, 파일 제어 가능.

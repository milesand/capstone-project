...
오늘 작업한 내용은 아래와 같습니다.
    ...
    1. 드래그박스가 안보이던 문제를 해결했습니다.
    2. 사이드바 축소시 로고크기가 커지는 오류를 해결했습니다.
    
6/20 수정사항
-----------------
오늘 작업한 내용은 아래와 같습니다.

    1. 프로필을 확인할 수 없었던 오류 수정 - 리액트 코드를 정리하다가 생긴 오류였습니다.
    2. fetch 구문을 axios 구문으로 변경 - 코드의 통일성과 브라우저 호환성을 위해 몇몇 fetch 구문을 axios 구문으로 바꿨습니다.
...
오늘 작업한 내용은 아래와 같습니다.
    ...
    3. 컨텐트 제목 사이즈 수정
    4. 팀컨텐츠 input 오류수정
    5. 업로드 이동 경로설정 modal 버튼들 다른 modal과 같게 통일
    6. 프로필 컨텐트 input 통일
    7. 모달 우측 상단 x표시 삭제
    8. 검색창 사이즈,위치 조정
    9. 파일.폴더 이름변경시 input 수정
    10. 초대 클릭시 x축 스크롤 없애기
    11. 길이가 너무 긴 modal들 길이 조정
 


해야할 내용은 아래와 같습니다.

    1. 드래그 박스 보이기(css로 보이게 하려했으나 작동안함)
    2. 초대 있으면 알수있도록 처리(ex)  초대아이콘 변경, 초대메시지 열려저 있는 상태로 시작)
...
오늘 작업한 내용은 아래와 같습니다.
    ...
    1. React 팀 관련 코드들을 정리했습니다.
    2. 팀 기능과 관련한 기능들의 오류를 수정했습니다.
...
오늘 작업한 내용은 아래와 같습니다.
    ...
    1. 팀별 공유폴더 목록을 보여주는 화면에서, 공유폴더의 소유자 이름을 보여주도록 설정했습니다.
    2. 업로드 창에서 업로드 속도 및 남은 시간, 진행도를 보여주도록 변경했습니다.
    3. 휴지통을 비우는 작업에서 RecycleEntry를 삭제할 때 연결된 DirectoryEntry가 같이 삭제되지 않는 현상을 발견했습니다. post_delete 시그널을 통해
       제거해주려 하였으나 recover에서 RecycleEntry를 지우는 과정에서 원본 파일도 같이 사라지는 현상이 발생하여 휴지통 비우는 작업에서 직접 
       수동으로 연결된 directoryEntry 모델 삭제해주도록 변경했습니다.
    4. 소셜로그인을 시도할 때 이미 해당 이메일로 가입한 사용자가 있을 경우, 로그인 못하도록 수정했습니다. 추후에 연동 여부를 확인하고 동의했을 경우 두 아이디를 연동하도록 바꿀 예정입니다.

만들어진 로고를 삽입하는 작업을 해야합니다. 

6/8 수정사항
-----------------
오늘 작업한 내용은 아래와 같습니다.

    1. 미리보기 이미지가 modal 밖으로 튀어나가던 현상을 수했습니다.
    2. 메일 인증 확인 페이지가 정렬되어있지 않은 현상을 수정했습니다.
    3. 모든 로딩 스피너를 사용하는 fetch 혹은 axios 문에서 에러 발생시 로딩 상태가 해제되도록 변경했습니다.
    4. 공유폴더에 있는 파일들 중 다른 사용자가 저장한 파일을 미리보기, 다운로드 할 수 없는 현상을 수정했습니다.
    5. 페이스북 로그인 버튼을 눌렀을 때 하얀색으로 변하는 현상을 수정랬습니다.
    6. 비밀번호 찾기 페이지에서 엔터를 눌렀을 떄 비정상적으로 작동하던 현상을 수정했습니다.
    7. 파일 및 디렉토리 이동 기능을 구현했습니다. 파일이나 디렉토리를 오른쪽 클릭한 뒤, 이동 버튼을 누르면 파일 브라우저를 통해 이동시킬 수 있습니다.
       현재 자신이 속한 디렉토리나, 자기 자신을 부모 디렉토리로 할 수 없습니다.
    8. 검색기능에 사용하기 위해 DirectoryEntry 모델에 get_browser_path 메소드를 추가했습니다. 이 메소드는 현재 파일이 사이트 상에서 저장되어 있는 경로를 리턴해줍니다.
    9. 검색기능을 추가했습니다. 검색기능은 현재 자신이 있는 폴더 안에 저장되어 있는 모든 파일 및 폴더들을 검사합니다.
       검색결과에서 파일이나 디렉토리를 클릭하면 오른쪽 정보창에서 실제 경로가 나타나며, 공유폴더에 있는 파일을 검색할 때는 공유폴더를 루트로 한 경로가 나타납니다.
       검색창을 빈칸으로 두고 검색하면 루트 디렉토리로 이동합니다.
       검색은 검색 디렉토리부터 시작해서 모든 하위 폴더 및 파일들을 단순한 BFS 탐색을 통해 이름에 키워드가 포함되어 있는지 검사하며, 따라서 효율이 좋지 않습니다. 추후에 개선이 필요합니다.
...
|URL          | 설명   |
|:--------------:|:-------:|
|/|메인 페이지입니다. 현재 저장중인 파일들을 확인할 수 있습니다.|
...
|/team|팀 관련 작업들을 처리하는 공간입니다.|
|/profile|사용자의 프로필을 확인할 수 있는 페이지입니다.|
|/sharing|공유 폴더를 확인할 수 있습니다. 현재 구현중입니다.|
|/trash|휴지통에 들어간 파일들을 확인하고 복원하거나 완전히 제거할 수 있습니다.|
|/favorite|즐겨찾기 설정한 파일들을 확인할 수 있습니다.|
...
|:--------------:|:-------:|:--------------------------: |:-------:|:---:|
| /users | - | 전체 사용자 출력 |      -     |-|
| /user| - | 해당 회원의 정보 출력 | 해당 회원의 정보 제거 |프로필 변경(비밀번호, 닉네임, 전화번호)|
| /registration | 회원가입 |-|-|-|
| /jwt-login |일반 로그인 |-|-|-|
| /social-login |소셜 계정 로그인 |-|-|-|
| /logout | 로그아웃 |-|-|-|
| /forgot | 아이디/비밀번호 찾기|-|-|-|
...
|                  | POST | GET | DELETE | PUT |
|:----------------:|:----:|:---:|:------:|:---:|
| /file/<id>       |-|현재 로그인한 사용자가 가지고 있는 특정 파일의 정보 출력|파일 삭제|파일 옵션 변경|
| /file-list       |-|현재 로그인한 사용자가 가지고 있는 전체 파일 정보 출력|-|-|
| /upload/flow     |파일 업로드 링크 생성|-|-|-|
| /upload/flow/<id>|chunk 전송|특정 chunk를 이미 전송받았는지 확인|-|-|
| /download        |파일 다운로드|-|-|-|
| /mkdir           |디렉토리 생성|-|-|-|
| /directory/<id>  |-|특정 디렉터리 하위 폴더 및 파일 정보 출력|디렉토리 삭제|디렉토리 옵션 변경|
| /thumbnail/<id>  |썸네일 생성|-|-|-|
| /partial/<id>    |-|-|특정 partial upload file 제거. 업로드를 중단할 때 사용|-|
| /multi-entry     |-|-|파일이나 폴더를 여러개 선택하여 한번에 삭제|-|
| /favorite        |즐겨찾기한 파일 목록 확인|-|-|-|
| /recycle         |-|사용자의 휴지통 내용 출력|휴지통 비우기|-|
| /recover         |파일 복원|-|-|-|
| /replacement     |-|-|-|파일이나 폴더 경로 변경|
| /preview         |-|이미지 및 동영상 미리보기에 사용할 파일 경로 반환|-|-|
| /search/<str:pk>/<str:keyword> |-|키워드를 통한 파일 및 폴더 검색|-|-|
| /sharing/(?P<pk>[0-9a-z-]{36} |-|-|공유 설정 해제|공유 설정|

* /file/<id>
  * GET 요청을 보내면, 현재 로그인한 사용자가 가지고 있는 특정 파일의 정보를 받을 수 있습니다.
  * DELETE 요청을 보내면 해당 파일을 삭제할 수 있습니다.
  * PUT 요청을 보내면 해당 파일의 옵션을 변경할 수 있습니다. 변경가능한 옵션은 파일 이름(`name` 필드)과 즐겨찾기 여부(`favorite` 필드)입니다.
  
* /file-list로 GET 요청을 보내면 현재 로그인한 사용자가 가지고 있는 전체 파일 정보를 받을 수 있습니다.

* /upload/flow는 업로드 시작 전에 파일의 크기를 서버로 보내어 사용자에게 할당된 공간에 파일을 저장할 수 있는지 확인합니다. POST 요청을 사용합니다.

* /upload/flow/<id>는 chunk를 전송하거나 특정 chunk를 이미 전송받았는지 확인합니다. 이는 flow.js에서 담당하므로 직접 요청을 보낼 필요는 없습니다.

* /download/<id>로 GET 요청을 보내면 그 파일 ID를 가진 파일을 다운로드 받을 수 있습니다.
...
* /directory/<id>
  * GET 요청을 보내면 해당 id를 가진 디렉토리의 정보를 반환합니다. 반환되는 정보의 형식은 다음과 같습니다.
    * 'pk`: 현재 디렉토리의 ID.
    * 'owner': 디렉토리 소유자의 ID.
    * 'name': 디렉토리의 이름.
    * 'parent': 상위 디렉토리의 ID.
    ...
    * `partial_uploads`: 해당 디렉토리에 진행중인 업로드의 목록; 이름을 키로 하며, id를 값으로 하는 딕셔너리.
    ...
    ```
  * DELETE 요청을 보내면 해당 디렉토리를 삭제합니다.
  * PUT 요청을 보내면 해당 디렉토리의 옵션을 변경할 수 있습니다. 변경가능한 옵션은 이름(`name` 필드), 즐겨찾기 여부(`favorite` 필드)입니다.

* /favorite에 GET 요청을 보내면 현 사용자가 즐겨찾기한 파일과 디렉토리 목록을 반환합니다. 각 디렉토리와 파일의 형식은 /directory/<id> 엔드포인트와 같습니다.

* /recycle
  * GET 요청을 보내면 사용자의 휴지통 내용을 반환합니다. 반환값은 `directories`와 `files` 키를 가지는 딕셔너리이며, 각 키에 대한 값은 이름을 키, 그 이름을 가지는 디렉토리/파일의 ID 목록을 값으로 하는 딕셔너리입니다.
  * DELETE 요청을 보내면 사용자의 휴지통을 비웁니다.

* /recover에 POST 명령을 보내면 휴지통의 파일을 복구합니다. POST body는 JSON 형식의 리스트여야 하며, 리스트의 각 값은 복구하려는 디렉토리/파일의 ID, 또는 ID와 복구 후 디렉토리/파일이 위치할 디렉토리의 ID 쌍이어야 합니다. 예를 들어, 다음과 같습니다:
  ```json
  [
    "SOME_UUID",
    ["ANOTHER_UUID", "DIRECTORY"]
  ]
  ```

...
5/7 수정사항
-----------------
 구글 로그인을 구현하여 로그인 폼에 추가하였습니다. 구현 과정에서 몇 가지 어려웠던 점들이 있었습니다.
...
|/mail-validation/*|인증 메일에 적힌 URL로 이동했을 때, 이 페이지로 오게됩니다. 여기서 메인 페이지로 이동할 수 있습니다.|

API

localhost/ 뒤에 아래의 URL을 붙이면 됩니다.

|           | POST    | GET        | DELETE  |
|:--------------:|:-------:|:--------------------------: |:-------:|
| api/users | - | 전체 사용자 출력 |      -     |
| api/user| - | 해당 회원의 정보 출력 | 해당 회원의 정보 제거 |
| api/registration | 회원가입 |-|-|
| api/jwt-login | 로그인 |-|-|
| api/logout | 로그아웃 |-|-|
| api/send-auth-email | 인증 메일 보내기 |-|-|
| api/forgot | 아이디/비밀번호 찾기|-|-| 
| api/jwt-refresh | JWT 토큰 재발급|-|-|
| api/jwt-verify | JWT 토큰 유효성 확인 |-|-|
| /active/<str:uidb64>/<str:token> |-| 사용자 이메일 인증 |-|

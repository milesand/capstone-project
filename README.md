...
4. /api/users에서 사옹자 정보에 추가로 각 사용자에게 연결된 UserStorage 정보를 보여주게 변경했습니다. 이 값을 통해서 루트 디렉토리의 ID를 알 수 있습니다.
...
|/upload-test|업로드 테스트페이지입니다. 로그인을 한 뒤 접근해야 합니다(현재는 테스트용으로 로그인 하지 않아도 접근 가능하게 설정해놓았습니다).|
API

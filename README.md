...
1. Storage 앱에 새로 업데이트된 내역들을 적용하고, 디버깅을 수행했습니다. 디버깅 과정에서 수정한 코드들은 아래와 같습니다.
    ...
    2. Directoryentry __init__에 있는 self.kind=self.KIND 각 하위모듈로 옮김 => partial upload에서 self.delete()할 때 자동 호출되서 오류생김.
    3. views.py 176~185번째줄 주석처리함 => partial_upload.uploader atrribute 없음
    4. Partial_upload 모델 is_expired 메소드 안에 있는 time_since_upload 수정 => now와 self.last_receive_time 형식 달라서 빼기가 안됨.
    5. PartialUpload 모델 is_complete => is_completed로 수정. signals.py와 맞춤.
    6. File 모델 path 메소드 수정 -> Path(settings.COMPLETE_UPLOAD_PATH, str(self.owner.pk), str(self.pk)) => Path(settings.COMPLETE_UPLOAD_PATH, str(self.owner.pk), str(self.pk) + PurePosixPath(self.name).suffix) (완성 파일에 확장자 추가)
    thumbnail_path도 확장자 붙도록 수정.
    7. 썸네일 생성부분(287번 줄)에서 완성 파일 경로로 file_path 재설정
    8. partial_upload의 complete 메소드에 file_record.save() 추가 (파일 모델 만들고 세이브 안했음.)
